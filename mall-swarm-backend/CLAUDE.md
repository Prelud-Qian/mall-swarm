...
- **认证链路**：mall-auth 用 OpenFeign 调 mall-admin（UmsAdminService）/ mall-portal（UmsMemberService）校验账号，签发 Sa-Token（token-name: `Authorization`、前缀 `Bearer`、存 Redis，sa-token-redis-jackson + JWT）。mall-gateway 用 sa-token-reactor 在网关层拦截校验：白名单在 gateway 的 `secure.ignore.urls`，路由拦截见 `SaTokenConfig`，权限加载见 `StpInterfaceImpl`。登录防爆破：`UmsAdminServiceImpl.login` 有 Redis 失败计数锁定（5 次锁 10 分钟，key `ums:admin:loginFail:{username}`），portal 会员登录未套同款。双凭证：accessToken 有效期 2 小时（`sa-token.timeout: 7200`），refreshToken 7 天（key `ums:admin:refreshToken:{UUID}`，换发接口 admin `/refreshToken`、auth `/auth/refresh`，旧 refreshToken 一次性作废）；前端 http.ts 拦截器 401 自动换发重试，`VITE_AUTH_SERVER_URL` 指向 auth 服务。
...
- **商品详情缓存（缓存三兄弟）**：`PmsPortalProductServiceImpl.detail` 套缓存外壳，key 为 `portal:product:detail:{id}`，value 为 JSON 完整详情。互斥锁（`lock:cache:product:{id}`）+ 双重检查防击穿；空值缓存（哨兵 `"NULL"`，5 分钟）防穿透；过期时间 30 分钟 + 随机 0~5 分钟防雪崩。缓存一致性：mall-admin 的 `PmsProductServiceImpl.update/updateDeleteStatus` 成功后删缓存 key（失效策略，两服务共用同一 Redis）。回源 `getDetailFromDb` 用 CompletableFuture 异步编排（product 串行根 + 6 路 supplyAsync 并行 + 属性值 thenApplyAsync 二级编排 + allOf().join()），线程池为 `BusinessThreadPoolConfig` 的 businessExecutor（线程名 businessPool-*，勿用 ForkJoinPool.commonPool）。
- **ES 检索增强（mall-search）**：综合搜索支持属性筛选（nested 查询，`attrValueList` 的 ID+值+type=1 同条件）与价格区间（range number 变体，filter 上下文）；`searchRelatedInfo` 聚合随品牌/分类筛选联动（条件并入 query，聚合基于结果集）；商品上下架 MQ 自动同步索引——admin `updatePublishStatus` 发 JSON 消息（队列契约见 mall-common 的 `ProductSyncMessage`/`SearchQueueEnum`），search 的 `ProductSyncReceiver` 消费（按 Map 字段解析），上架 `create` 建索引、下架 `delete` 删索引。注意 elasticsearch-java 8.18 的 RangeQuery 是 TaggedUnion，数值区间走 `number` 变体。
- **可靠消息投递（本地消息表 Outbox）**：上下架消息改为事务内落 `mall_local_message` 表 → afterCommit 发送 → Publisher Confirms 确认才 markSent → `LocalMessageResendTask` 每 30 秒补发（重试上限 5）。注意：`AmqpTemplate` 接口没有带 CorrelationData 的重载（用 `RabbitTemplate`）；Broker 对不可路由消息先 return 后 ack，确认回调需防时序覆盖；RabbitMQ 对不存在交换机静默丢弃，必须 mandatory+confirms 才能感知失败。
...
- **Seata 分布式事务演示**：Seata Server 2.0.0 本机运行（D:\seata-run，start.bat 启动，RPC 8091/控制台 7091，file 存储），mall 库有 undo_log 表；demo 服务 `SeataDemoServiceImpl.txBrand` 标 `@GlobalTransactional` 发起（本地写品牌+Feign 调 portal `/brand/txDemo`），portal 为 RM 参与者；两服务 yml 的 seata 配置含 `grouplist.default=127.0.0.1:8091`。注意：Feign 对 HTTP 200+code500 不抛异常，远程业务失败必须检查返回码手动抛出才能触发全局回滚。
